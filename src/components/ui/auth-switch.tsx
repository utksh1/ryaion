...
import { Lock, User, Mail, Loader2, CheckCircle2, ArrowLeft, AlertCircle } from "lucide-react";
...
export const AuthSwitch = ({ onSuccess, onBackToDashboard }: AuthSwitchProps) => {
    ...
    const socialIcons = [
        { Icon: GoogleIcon, title: "Google", provider: 'google' },
        { Icon: FacebookIcon, title: "Facebook", provider: 'facebook' },
        { Icon: XIcon, title: "X", provider: 'twitter' },
        { Icon: LinkedinIcon, title: "LinkedIn", provider: 'linkedin' }
    ] as const;
                                ...
                                >
                                    ...
                                    <div className="flex gap-4">
                                        {socialIcons.map(({ Icon, provider }, idx) => (
                                            <div key={idx} onClick={() => handleSocialAuth(provider as any)} className="w-[72px] h-[72px] bg-white/[0.03] border border-white/5 rounded-3xl flex items-center justify-center cursor-pointer hover:border-white/20 hover:bg-white/10 transition-all active:scale-95 group relative overflow-hidden">
                                                <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                                                <div className="relative group-hover:scale-110 transition-transform duration-300 filter grayscale group-hover:grayscale-0 opacity-40 group-hover:opacity-100">
                                                    <Icon />
                                                </div>
                                            </div>
                                        ))}
